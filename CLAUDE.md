...
- **Over-certification (QS certifies ABOVE application, 2026-08-18)**: cert modal flags certified > applied lines in amber (payless stays red) with a status note. Next-AFP seeding: when a line's paid-to-date exceeds its cum applied value, `previous_pct_complete` (and the default this-app %) is lifted to `paid/contract_value` (2dp, may exceed 100) so paid value is never re-applied; payless direction deliberately keeps our applied % so shortfalls stay claimed. Warning toast lists lifted lines on AFP open.
- **Lump distribution is deterministic (2026-08-18 fix)**: the AI NEVER splits/scales/caps values — it returns `breakdown_lumps` [{description, certified_value (printed), line_indices}] matching each QS lump to our lines; JS distributes pro-rata to CONTRACT VALUE (never applied value — earlier prompt wrongly capped over-certified lines at applied), last line takes the rounding remainder so lines sum exactly to the lump; explicit per-line `line_items` win, lumps fill only blank inputs; summary auto-fill only when neither produced anything.
